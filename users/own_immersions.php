...
$stmt = $bdd->prepare("SELECT * from immersions WHERE nni = :nni ");
...
<head>
    <meta charset="UTF-8">
    <title>Mon Immersion - Mes Immersions</title>
...
<?php if (empty($immersions)) : ?>
  <p class="aucun">Vous n'avez pas encore demandé d'immersions, <a href="../users/accueil.php" class="go"> J'y vais ! </a></p>
<?php else : ?>
  <table>
      <tr>
          <th>NNI</th>
          ...
          <th>Heure de fin</th>
          <th onclick="openModal(event, 'modalId')">Note</th>
          <th>Statut</th>
      </tr>
      ...
      <?php foreach ($immersions as $immersion) : ?>
        ...
        <tr>
            ...
            <td><?= date('d-m-Y', strtotime($immersion['date_immersion'])); ?></td>
            <td>
              <?php
          ...
          <td>
            ...
            <td>
            <?php if (empty($immersion['note']) || $immersion['note'] === '/') : ?>
                <?php if ($immersion['statut'] === 'Approuvée par le manager' && strtotime($immersion['date_immersion']) < strtotime('today')) : ?>
                    <button class="note-button" onclick="openModal(event, <?= $immersion['id_immersion']; ?>)" data-immersion-id="<?= $immersion['id_immersion']; ?>" data-immersion-date="<?= date('d-m-Y', strtotime($immersion['date_immersion'])); ?>" data-immersion-nni="<?= $immersion['nni']; ?>">Noter</button>
                <?php else : ?>
                    <span>/</span>
                <?php endif; ?>
            <?php else : ?>
                <div class="rating">
                    <?php $note = intval($immersion['note']); ?>
                    <?php for ($i = 1; $i <= 5; $i++) : ?>
                        <?php if ($i <= $note) : ?>
                            <span class="etoile">&#9733;</span>
                        <?php else : ?>
                            <span class="etoile_non">&#9733;</span>
                        <?php endif; ?>
                    <?php endfor; ?>
                </div>
            <?php endif; ?>
...
<div id="modalId" class="modal">
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <p class="avis">Donnez votre avis (suggestions, remarques, etc..)</p>      
      
      <div class="info-container">
        <img class="info-icon" src="../img/info.png">
        <div class="hover-text">
          En soumettant un commentaire, vous acceptez de vous conformer à ne pas écrire de commentaires offensants, abusifs ou contenant des informations personnelles ou sensibles.
          Si votre commentaire ne respecte pas ces règles, il sera supprimé.
        </div>
      </div>
      <br>

      <input id="confirmation" type="hidden">
...
<script>
    document.getElementById("exportButton").addEventListener("click", function() {
        ...
        for (var i = 1; i < tableRows.length; i++) {
            ...
            rowData.push(feedbackText); // Feedback
            
            rowData.push(cells[8].textContent); // Statut
        ...
        XLSX.writeFile(wb, "Export_Mes_Immersions.xlsx");
    });
...
$(document).ready(function() {
  ...
  $(".star").click(function() {
    ...
    $("#confirmation").val(selectedRating);
  });

  $(".note-button").click(function() {
    var immersionId = $(this).attr("data-immersion-id");
    var immersionDate = $(this).attr("data-immersion-date");
    var immersionNni = $(this).attr("data-immersion-nni");

    selectedRating = 0;
    comment = "";

    openModal(null, 'modalId');

    $("#modalId").data("immersion-id", immersionId);
    $("#modalId").data("immersion-date", immersionDate);
    $("#modalId").data("immersion-nni", immersionNni);
  });
  ...
  $("#confirmButton").click(function () {
    ...
    var immersionId = $("#modalId").data("immersion-id");
    var immersionDate = $("#modalId").data("immersion-date");
    var immersionNni = $("#modalId").data("immersion-nni");

    $.ajax({
        url: "enregistrer_note.php",
        method: "POST",
        data: {
            nni: immersionNni,
            date_immersion: immersionDate,
            note: confirmation,
            commentaire: comment
        },
        success: function (response) {
            console.log("Note et commentaire enregistrés avec succès !");
            var noteSpan = $("button[data-immersion-id='" + immersionId + "']").siblings(".rating");
            noteSpan.html('<span class="star selected">&#9733;</span>'.repeat(confirmation));
            localStorage.setItem("toastNeeded", "true");
            location.reload();
        },
        error: function (xhr, status, error) {
            console.error("Erreur lors de l'enregistrement de la note et du commentaire : " + error);
        }
    });
  });

  if (localStorage.getItem("toastNeeded") === "true") {
    Toastify({
        text: "Note et Commentaire enregistrés avec succès",
        duration: 8000,
        close: true,
        gravity: "top",
        position: 'center',  
    ...
    localStorage.setItem("toastNeeded", "false");
  }
});
